...
loadDataUserSiga = async () => {
  ...
  localizaMotoraSiga = async () => {
    const response = await api.get('/runs/7/driver_location');
    const  localiza  = response.data;
    console.log(response.data);
  
    
    this.setState({ localiza: localiza });
...
<TouchableOpacity style={styles.productButton} onPress={() =>{
  this.props.navigation.navigate("sigaUser", { user: item } );
}}>
...
<View style={styles.container}>
    <FlatList 
      ...
      renderItem={this.renderDataUser}
